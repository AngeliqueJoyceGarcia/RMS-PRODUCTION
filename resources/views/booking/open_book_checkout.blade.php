...
<div class="body">
    ...
    <div class="upper">
        <div class="containerUsers">
            <span class="circlebackground">{{ isset($bookings) ? count($bookings) : 0 }}</span>Check-out
        </div>
        ...
        <div class="containerArchives">
            <a class="archive" href="{{ auth()->user()->role_id === 1 ? route('openbook.checkin') : route('reguser.openbook.checkin') }}">Check-in</a>
        </div>
    </div>
        ...
        @php
            ...
            @foreach($reversedBookings as $booking)
                @foreach($booking->billings as $billing)
                    @if ($billing->reservation_type === 'Open-book' && $billing->status === 'check-in')
                        <tr>
                            <td>{{$billing->name }}</td>
                            <td>{{$billing->bday }}</td>
                            ...
                            <td>
                                <div class="popup">
                                    ...
                                    <div class="popup-content">
                                        <form action="{{ auth()->user()->role_id === 1 ? route('openbook.checkout_edit', ['billing' => $billing->id]) : route('reguser.openbook.checkout_edit', ['billing' => $billing->id]) }}">
