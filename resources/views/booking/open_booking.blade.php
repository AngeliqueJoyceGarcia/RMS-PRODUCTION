...
<div class="body">
    ...
    <div class="upper">
        <div class="containerUsers">
            <span class="circlebackground">{{ isset($bookings) ? count($bookings) : 0 }}</span>Open-Bookings
        </div>
        ...
        @php
            ...
            @foreach($reversedBookings as $booking)
                @foreach($booking->billings as $billing)
                    @if ($billing->reservation_type === 'Open-book' && ($billing->status === 'Open booking' || $billing->status === 'pending'))
                        <tr>
                            <td>{{$billing->name }}</td>
                            <td>{{$billing->bday }}</td>
                            ...
                            <td>
                                <div class="popup">
                                    ...
                                    <div class="popup-content">
                                        <form action="{{ auth()->user()->role_id === 1 ? route('openbook.edit', ['billing' => $billing->id]) : route('reguser.openbook.edit', ['billing' => $billing->id]) }}">
