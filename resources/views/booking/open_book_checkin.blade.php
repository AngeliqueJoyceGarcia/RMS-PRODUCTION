...
<div class="body">
    ...
    <div class="upper">
        <div class="containerUsers">
            <span class="circlebackground">{{ isset($bookings) ? count($bookings) : 0 }}</span>Check-in
        </div>

        <div class="containerArchives">
            <a class="archive" href="{{ auth()->user()->role_id === 1 ? route('openbook.checkout') : route('reguser.openbook.checkout') }}">Check-out</a>
        </div>

        <div class="containerArchivess">
            <a class="archive" href="{{ auth()->user()->role_id === 1 ? route('openbook.read') : route('reguser.openbook') }}">Open-Bookings</a>
        </div>
    </div>
        ...
        @php
            ...
            @foreach($reversedBookings as $booking)
                @foreach($booking->billings as $billing)
                    @if ($billing->reservation_type === 'Open-book' && $billing->status === 'Open-book' || $billing->status === 'pending')
                        <tr>
                            <td>{{$billing->name }}</td>
                            <td>{{$billing->bday }}</td>
                            <td>{{$billing->total_companion  }}</td>
                            <td>{{$billing->arrived_companion }}</td>
                            <td>{{$billing->total_prebookAmount }}</td>
                            <td>{{$billing->refundablePrice }}</td>
                            <td>{{$billing->total_amount }}</td>
                            <td>{{$billing->total_amount_paid }}</td>
                            <td>{{$billing->status }}</td>
                            <!-- defaul zero, will updated when refund is clicked -->
                            <td>{{ $billing->remarks }}</td>
                            <td>
                                <div class="popup">
                                    <button class="css-button">...</button>
                                    <div class="popup-content">
                                        <form action="{{ auth()->user()->role_id === 1 ? route('openbook.checkin_edit', ['billing' => $billing->id]) : route('reguser.openbook.checkin_edit', ['billing' => $billing->id]) }}">
                                            <button class="popup-button">Edit</button>
                                        </form>
                                        <form action="{{ auth()->user()->role_id === 1 ? route('booking.cancelBook', ['booking' => $booking]) : route('reguser.cancelBook', ['booking' => $booking]) }}">
                                            @csrf 
                                            <button type="submit" class="popup-button">Cancel Booking</button>
                                        </form>
                                    </div>
                                </div>
                            </td>
