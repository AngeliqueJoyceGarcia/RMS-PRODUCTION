...
<div class="body">
    ...
    <div class="upper">
        <div class="containerEntrance">
            <span class="circlebackground">{{ isset($rates) ? count($rates) : 0 }}</span>Deleted
        </div>

        <div class="containerArchives">
            <a class="archive" href="{{ route('rates.read')}}">Rates</a>
        </div>
    </div>
    ...
    <tbody>
        @foreach($rates as $rate)
            <tr>
                ...
                <td>
                        <form method="POST" action="{{ route('rates.restore', ['rate'=> $rate->id]) }}">
                                @csrf
                                @method('POST')
    ...
    // Download excel
                        document.addEventListener("DOMContentLoaded", function() {
                              ...
                              downloadButton.addEventListener("click", function() {
                                  ...
                                  XLSX.writeFile(wb, "DeletedEntranceRates.xlsx");
                              });
                          });
