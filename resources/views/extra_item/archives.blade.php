...
<div class="body">
    ...
    <div class="upper">
        <div class="containerExtras">
            <span class="circlebackground">{{ isset($extras) ? count($extras) : 0 }}</span> Deleted
        </div>
        <div class="containerArchives">
            <a class="archive" href="{{ route('extras.read')}}">Extra Items</a>
        </div>
    </div>
    ...
    <tbody>
        @foreach($extras as $extra)
            <tr>
                ...
                <td>{{ $extra->price }}</td>
                <td>
                        <form method="POST" action="{{ route('extras.restore', ['extra'=> $extra->id]) }}">
                                @csrf
                                @method('POST')
    ...
    // Download excel
                        document.addEventListener("DOMContentLoaded", function() {
                              ...
                              downloadButton.addEventListener("click", function() {
                                  ...
                                  XLSX.writeFile(wb, "DeletedExtraItems.xlsx");
                              });
                          });
