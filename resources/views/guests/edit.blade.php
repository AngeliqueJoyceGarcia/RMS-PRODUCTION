...
<div class="body">
    ...
    <div class="popup-container" id="popupContainer">
      ...
      <hr>
    <form action="{{ route('guests.update', ['id' => $guest->id]) }}" method="POST">
        @csrf
        ...
        <div class="extra-details">
                ...
                <div class="form-group">
                    <label for="status">Status:<span class="edit">(update)</span></label>
                    <div class="dropdown-select">
                        <select class="dpfontsize" id="status" name="status" readonly>
                            <option value="confirmed" {{ $guest->booking->status === 'confirmed' ? 'selected' : '' }}>Confirmed</option>
                            <option value="pending" {{ $guest->booking->status === 'pending' ? 'selected' : '' }}>Pending</option>
                        </select>
                ...
                <div class="form-group">
                    <label for="arrivedCompanion">Arrived Companion:<span class="edit">(update)</span></label>
                    <input type="number" id="arrivedCompanion_{{ $guest->id }}" name="arrivedCompanion" placeholder="Arrived Companion" 
                        value="{{ $guest->booking->arrived_companion }}" min="0" max="{{ $guest->booking->total_companion }}">
        ...
        <br>
        <button class="ExtraButton" type="submit"> <span>Update</span></button>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        const arrivedCompanionInputs = document.querySelectorAll("[id^='arrivedCompanion_']");

        arrivedCompanionInputs.forEach(arrivedCompanionInput => {
            arrivedCompanionInput.addEventListener("input", function() {
                const arrivedCompanionValue = parseInt(arrivedCompanionInput.value);
                const totalCompanionValue = parseInt(arrivedCompanionInput.getAttribute('max'));

                if (arrivedCompanionValue < 0) {
                    arrivedCompanionInput.value = 0;
                } else if (arrivedCompanionValue > totalCompanionValue) {
                    arrivedCompanionInput.value = totalCompanionValue;
                }
            });
        });
    });
